(() => {
  ...
  function buildItemJsonPath(workSlug, entrySlug) {
    return `${WORKS_DIR}/${encodeURIComponent(workSlug)}/${encodeURIComponent(entrySlug)}/${ITEM_JSON_NAME}`;
  }
  ...
  async function loadWork(dir, file) {
    ...
    const data = await fetchJson(buildItemJsonPath(resolved.dir, resolved.file));
    ...
    if (titleEl) {
      titleEl.textContent = `${CURRENT_WORK_TITLE} • ${CURRENT_CHUNK_TITLE}`;
    }
  ...
  function getUiPageLabel(page) {
    ...
    const startNum = Number.isFinite(startItem?.page) ? startItem.page : startIdx + 1;
    const endNum = Number.isFinite(endItem?.page) ? endItem.page : endIdx + 1;
  ...
  // ADS
  // =========================
  function buildBetweenAd(slots = 3) {
    ...
    for (let i = 0; i < slots; i++) {
      ...
      box.textContent = `Between Ad ${i + 1}`;
    ...
    d.innerHTML = `
      <summary>
        ...
        <div class="action-open">
          <a class="pill" href="${escapeHtml(item.url)}" target="_blank" rel="noopener">OPEN</a>
          <div class="open-note">Opens image in new tab</div>
        </div>
      </summary>
  ...
  document.addEventListener("toggle", (e) => {
    ...
    if (d.open) {
      if (expBtn) expBtn.textContent = "▲ HIDE";
      ...
      if (expBtn) expBtn.textContent = "▶ READ";
  ...
  async function boot() {
    try {
      ...
      const meta = $("#meta");
      if (meta) {
        meta.textContent = `Load error: ${err.message}`;
      }
    }
  }
